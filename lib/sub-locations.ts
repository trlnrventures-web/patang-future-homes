...
function collectSubLocations(): string[] {
  ...
  for (const m of rawMarketInventory as WithSubLocation[]) add(m.subLocation);
 ...
 * so the website filter, CRM property form, lead requirement form and the
 * matching engine can never drift out of sync with the actual data.
 */
